...
| 技术 | 版本 | 用途 | 状态 |
|------|------|------|------|
| **@google/genai** | ^1.43.0 | Google Gemini AI API | ⚠️ 需配置 API 密钥 |

> **注意**: AI 功能为可选特性，需要设置 `GEMINI_API_KEY` 环境变量才能启用。未配置时游戏仍可正常运行，AI 机器人将使用预设的快捷回复。
...
### 4. AI 聊天系统 (src/services/geminiService.ts)

#### 4.1 功能概述
AI 聊天系统为可选功能，用于让 AI 机器人能够智能回复玩家的聊天消息。

#### 4.2 触发场景
1. **玩家聊天回复**: 当人类玩家发送聊天消息时，随机一个 AI 机器人会生成回复
2. **游戏事件反应**: 当玩家打出炸弹或火箭时，AI 会做出反应

#### 4.3 AI 角色设定
- AI 扮演韩国女团 (G)I-DLE 成员
- 使用中文回复（可混合简单韩语）
- 回复风格：活泼、竞争性或可爱
- 回复长度：1-2 句话或几个词
...
#### 4.5 降级处理
- 当 API 密钥未配置或调用失败时，AI 不会发送消息
- 游戏核心功能不受影响
- 建议：可以添加预设的快捷回复作为降级方案
